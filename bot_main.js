...
const ARB_AGAINST1 = process.env.ARB_AGAINST1 // SHIB
const ARB_AGAINST2 = process.env.ARB_AGAINST2 // PLSD
...
const main = async () => {
    const { token0Contract, token1Contract, token0, token1 } = await getTokenAndContract(arbFor, ARB_AGAINST1)
    const token3Contract = (await getTokenAndContract(arbFor,ARB_AGAINST2)).token0Contract
    const token4Contract = (await getTokenAndContract(arbFor,ARB_AGAINST2)).token1Contract
    const token2 = (await getTokenAndContract(arbFor,ARB_AGAINST2)).token1
    ...
    uPair2 = await getPairContract(uFactory, token0.address, token2.address)
    sPair2 = await getPairContract(sFactory, token0.address, token2.address)
    ...
    uPair2.events.Swap({}, async () => {
        if (!isExecuting) {
            ...
            const priceDifference = await checkPrice('Uniswap', token0, token2)
            const routerPath = await determineDirection(priceDifference)
            ...
            const isProfitable = await determineProfitability(routerPath, token3Contract, token0, token2)
            ...
            const receipt = await executeTrade(routerPath, token3Contract, token4Contract)
    ...
    sPair2.events.Swap({}, async () => {
        if (!isExecuting) {
            ...
            const priceDifference = await checkPrice('Sushiswap', token0, token2)
            const routerPath = await determineDirection(priceDifference)
...
const checkPrice = async (exchange, token0, token1) => {
    ...
    const uPrice = await calculatePrice(uPair1)
    const sPrice = await calculatePrice(sPair1)
...
const determineProfitability = async (_routerPath, _token0Contract, _token0, _token1) => {
    ...
    if (_routerPath[0]._address == uRouter._address) {
        reserves = await getReserves(sPair1)
        exchangeToBuy = 'Uniswap'
        exchangeToSell = 'Sushiswap'
    } else {
        reserves = await getReserves(uPair1)
