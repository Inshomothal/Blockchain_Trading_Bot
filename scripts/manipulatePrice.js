...
const GAS_TEST = bn.toNumber(web3.eth.getGasPrice().then((result) => {
    web3.utils.toWei(result, 'wei')
}))
...
async function manipulatePrice(tokens, account) {
    const GAS = web3.utils.fromWei(await web3.eth.getGasPrice(), "kwei")
    console.log('GAS is a ' + typeof(GAS) + ' value.' + " It's the value of " + GAS + ".")
    console.log('GAS_TEST is a ' + typeof(GAS_TEST) + ' value.' + " It's the value of " + GAS_TEST + ".")
